// vClock script: clock, alarm, timer, stopwatch with settings persistence
...
(() => {
  ...
  function renderAlarms(){
    ...
    alarms.forEach((a, i) => {
      ...
      title.innerHTML = `<strong>${a.label || 'Alarm'}</strong>`;
      const meta = document.createElement('small');
      meta.textContent = `${a.time} • ${a.repeat}`;
  ...
  function playAlarmSound(){
    ...
    o1.connect(merger); o2.connect(merger); merger.connect(g); g.connect(ctx.destination);
    g.gain.value = 0;
  ...
  function triggerAlarm(alarmObj){
    ...
    alarmPlaying = true;
    playAlarmSound();
    // simple confirm UI (keeps consistent cross-browser)
    const stop = confirm(`${alarmObj.label || 'Alarm'}\n\nStop alarm?`);
    stopAlarm();
  }
  function stopAlarm(){ alarmPlaying = false; /* WebAudio ramps down; nothing else to stop */ }
  ...
  resetTimerBtn.addEventListener('click', () => {
    ...
    timerRemaining = 0; renderTimer(); timerPausedRemaining = null;
  });
  ...
  swReset.addEventListener('click', () => {
    swRunning = false; clearInterval(swInterval);
    swElapsed = 0; swStartTs = 0; lapsEl.innerHTML = ''; renderStopwatch();
  });
  ...
  swLap.addEventListener('click', () => {
    ...
    const mm = Math.floor(s / 60);
    const ss = s % 60;
    const li = document.createElement('li'); li.textContent = `${pad(mm)}:${pad(ss)}.${pad(ms)}`;
    lapsEl.prepend(li);
  });
  ...
  window.vClockCloneEmbed = {
    setAlarmLink: (hhmm, label = 'Alarm') => `${location.origin}${location.pathname}?setAlarm=${encodeURIComponent(hhmm)}&label=${encodeURIComponent(label)}`
  };

  // query param handler (set alarm via URL)
